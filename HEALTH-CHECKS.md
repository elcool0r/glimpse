...
| Filesystem space and inodes (`statfs`, `/proc/self/mountinfo`) | Read-only mounts are reported as informational when they are not an expected image filesystem. Utilization is measured against space available to a normal process, matching `df`, so the root reserve is not counted as free. `tmpfs` is included: it is RAM-backed but genuinely mountable with a fixed size and can fill up exactly like a disk-backed filesystem. Capacity is counted once per underlying filesystem (mountinfo's `major:minor` plus the mounted subtree), not once per mount point: `statfs` returns identical numbers for every bind mount of one device, so counting per mount point turned a single full device bind-mounted at three paths into three independent criticals that read like three separate problems | Space ≥85% used; inode use ≥90% | Space ≥95% used; inode use ≥98% |
...
| SELinux / AppArmor state | Enforcing/enabled and unknown states are summarized. Denials are counted from the kernel journal over the same one-hour window | SELinux permissive (a loaded-but-not-enforcing policy is host configuration the operator can change); SELinux or AppArmor denials in the window | None |
| Kernel vulnerability files (`/sys/devices/system/cpu/vulnerabilities`) | A status beginning with `Vulnerable` is reported as evidence with no score impact: it is fixed for the life of the running kernel and microcode, identical on every run, and on current kernels a great many well-maintained hosts report at least one that no action on this host can clear | None | None |
...
| `ssh` | Real interactive login sessions (user, source address) from wtmp via `last --time-format=iso -i` | wtmp is only written for a session that allocates a login shell, so a non-interactive `ssh host command` invocation -- which allocates no pty -- has no wtmp entry and is excluded by construction. `last` cannot reveal the authentication method, so it is never populated |
| `sudo` | Interactive sudo command executions (user, target user, command) from the authpriv journal | Scoped to commands run from a real terminal: sudo logs `TTY=unknown` (or omits `TTY`) for a cron job or script with no controlling terminal, and those are excluded the same way a non-interactive SSH session is |
...
Severity and score impact are two halves of one contract, and they are kept in
step deliberately. The highest severity present becomes the report's status,
which is what the process exit code is derived from; the summed impacts become
its numeric score. A `WARN` or `CRIT` finding that subtracts nothing would make
the two disagree permanently -- a report reading 100/`EXCELLENT` while the exit
code said otherwise -- so no rule ships in that state, and the label is never
more favourable than the highest severity allows.

The practical consequence is that anything which is fixed for the life of the
running kernel, identical on every run, and not actionable from this host is
reported as `INFO` rather than `WARN`. CPU vulnerability status and kernel
taint are both in that class.
