...
| Network activity | Local kernel counters, sockets, routes, and logs only, plus four small active probes on by default: DNS resolution against the configured local nameserver and against `1.1.1.1`, an ICMP ping to the default gateway, a path-MTU black-hole probe against `1.1.1.1`, and an HTTP/HTTPS GET to `example.com`. `--disable-external-checks` turns all four off and falls back to purely passive collection |
...
| Filesystem space and inodes (`statfs`, `/proc/self/mountinfo`) | Read-only mounts are reported as informational when they are not an expected image filesystem. Utilization is measured against space available to a normal process, matching `df`, so the root reserve is not counted as free | Space ≥85% used; inode use ≥90% | Space ≥95% used; inode use ≥98% |
...
Four checks send real packets instead of only reading local state. All four
are on by default; `--disable-external-checks` turns them off.
...
| DNS resolution, external (`1.1.1.1`) | Resolved name, server, and latency are shown in verbose output | Resolution fails against `1.1.1.1` while the local nameserver succeeds; this is common on hosts with restricted outbound access and is informational, not a warning | Both the local and external resolution fail (or the external one fails and no local nameserver is configured): the host cannot currently resolve names |
| Gateway ping (`internal/collect/gatewayping`) | Sent/received counts and average latency of a small ICMP echo series to the default gateway, run via the system `ping` binary (bounded, timed out, output parsed regardless of exit code since ping exits nonzero on any loss). A host without a `ping` binary reports the check unavailable rather than failing or warning. Skipped entirely when no default route is configured | Majority packet loss (≥50%) | No reply at all |
| Path MTU discovery (`internal/collect/pathmtu`) | An ordinary baseline ping to `1.1.1.1`, then a descending series of non-fragmentable pings (`ping -M do -s <size>`, 1472 down to 548 bytes), reporting the largest size that got through. The full 1500-byte ceiling working is quiet; a smaller size getting through is a reduced but working path MTU, normal and healthy with PPPoE, VPNs, or tunnels when path MTU discovery (PMTUD) is working. Skipped entirely when the anchor host is unreachable at all (a connectivity fact the gateway/DNS checks already cover) or when the local `ping` does not support `-M do` | No size at all got through despite the baseline succeeding: PMTUD itself is not working (commonly because the ICMP "fragmentation needed" replies it depends on are filtered), which stalls large transfers while small traffic and pings work fine | None |
| HTTP/HTTPS GET (`internal/collect/httpcheck`) | A GET to `http://example.com/` and to `https://example.com/` succeeding (any status code, since this tests reachability, not content) is quiet; latency and status code are shown in verbose output. Every failure combination (HTTP only, HTTPS only, or both) is reported here, at informational severity, because an intentionally restricted host (firewall, proxy, air-gapped network) produces the identical signal to a genuine fault | None; never escalated past informational | None |
